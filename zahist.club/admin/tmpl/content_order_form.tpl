					...
					<td>
						<select name="products_<?=$i?>">
							<?php for ($j = 0; $j < count($this->products_all); $j++) { ?>
								<option value="<?=$this->products_all[$j]["id"]?>" <?php if ($this->products_all[$j]["id"] == $this->products[$i]["id"]) { ?>selected="selected"<?php } ?>><?=$this->products_all[$j]["id"]?> <?=$this->products_all[$j]["title"]?> (<?=$this->products_all[$j]["model"]?>)</option>
							<?php } ?>
						</select>
					</td>
					<td>
